 ...
 * @author qinghai
 */
public class SimpleWaterMain extends SimpleApplication {
    ...
    private AbstractHeightMap loadRandomHeightMap() {
        try {
            ...
            Logger.getLogger(SimpleWaterMain.class.getName()).log(Level.SEVERE, null, ex);
            return null;
        }
    }
    ...
    private Spatial loadWarter() {
        SimpleWaterProcessor waterProcessor = new SimpleWaterProcessor(assetManager);
        waterProcessor.setReflectionScene(reflectedScene);
        waterProcessor.setLightPosition(lightDir.mult(-3000));
        viewPort.addProcessor(waterProcessor);
        
        Spatial waterPlane = waterProcessor.createWaterGeometry(1000, 1000);
        waterPlane.setMaterial(waterProcessor.getMaterial());
        waterPlane.setLocalTranslation(-500, 0.5f, 500);
        return waterPlane;
    }
...
//        rootNode.addLight(ambient);
    ...
    public void simpleInitApp() {
        ...
        reflectedScene.attachChild(loadJaime());
        
        rootNode.attachChild(reflectedScene);
        
        rootNode.attachChild(loadWarter());
    }

    public static void main(String[] args) {
        AppSettings settings = new AppSettings(true);
        settings.setSettingsDialogImage(null);
        SimpleWaterMain app = new SimpleWaterMain();
